...
class Set(Tk):
    def __init__(self):
        ...
        def verify():
            if len(d.get()) == 0:
                messagebox.showinfo("Wrong","Please Enter Your User Id")
            elif len(a.get()) < 4 and len(b.get()) < 4 and len(c.get()) < 4:
                messagebox.showinfo("Wrong","Please Enter a Valid Password")
            elif b.get() != c.get():
                ...
                try:
                    ...
                    if temp:
                        if str(temp[0]) == a.get():
                            ...
                            self.conn.commit()
                            self.conn.close()
                            messagebox.showinfo("Executed","Password Updated successfully")
                        else:
        ...
        butt=Button(self,text="Change",width=15,command = verify).place(x=270,y=255)
Set().mainloop()