...
class Add(Tk):
    def __init__(self):
        ...
        def b_q():
            if len(a.get()) == 0 or len(b.get()) == 0:
                ...
                try:
                    ...
                    if ask:
                        self.destroy()
                        os.system('%s %s' % (py, 'Add_NewBooks.py'))
                    else:
                        self.destroy()
                except Error:
        ...
        Label(self, text='Book Title:', font=('Comic Scan Ms', 10, 'bold')).place(x=100, y=180)
        Entry(self, textvariable=b, width=30).place(x=230, y=182)
        Label(self, text='Author:', font=('Comic Scan Ms', 10, 'bold')).place(x=100, y=230)
